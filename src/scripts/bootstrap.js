...
+ function ($) {
	...
	$.fn.emulateTransitionEnd = function (duration) {
		...
		var callback = function () {
			if (!called) $($el)
				.trigger($.support.transition.end)
		}
...
+ function ($) {
	...
	$(document)
		.on('click.bs.alert.data-api', dismiss, Alert.prototype.close)
...
+ function ($) {
	...
	Carousel.prototype.to = function (pos) {
		...
		if (activeIndex == pos) return this.pause()
			.cycle()
...
+ function ($) {
	...
	var Collapse = function (element, options) {
		...
		this.$trigger = $(this.options.trigger)
			.filter('[href="#' + element.id + '"], [data-target="#' + element.id + '"]')
...
+ function ($) {
	...
	Dropdown.prototype.keydown = function (e) {
		...
		if ((!isActive && e.which != 27) || (isActive && e.which == 27)) {
			if (e.which == 27) $parent.find(toggle)
				.trigger('focus')
...
+ function ($) {
	...
	Modal.prototype.resize = function () {
		if (this.isShown) {
			$(window)
				.on('resize.bs.modal', $.proxy(this.handleUpdate, this))
		} else {
			$(window)
				.off('resize.bs.modal')
		}
...
+ function ($) {
	...
	Tooltip.prototype.toggle = function (e) {
		...
		self.tip()
			.hasClass('in') ? self.leave(self) : self.enter(self)
	}
...
+ function ($) {
	...
	Popover.prototype.setContent = function () {
		...
		if (!$tip.find('.popover-title')
			.html()) $tip.find('.popover-title')
			.hide()
	}
	...
	ScrollSpy.prototype.refresh = function () {
		...
		this.$body
			...
			.map(function () {
				...
				return ($href && $href.length && $href.is(':visible') && [[$href[offsetMethod]()
					.top + offsetBase, href]]) || null
			})
			.sort(function (a, b) {
				return a[0] - b[0]
			})
...
+ function ($) {
	...
	Tab.prototype.activate = function (element, container, callback) {
		...
		var transition = callback && $.support.transition && (($active.length && $active.hasClass('fade')) || !!container.find('> .fade')
			.length)
